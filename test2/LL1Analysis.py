...
class LL1Grammar:
    ...
    def __subFirst(self, X):
        tmpSize = len(self.First[X])  # 求之前的大小(每次都会更新first集)
        if X in self.Terminater and X not in self.First[X]:  # 如果X在终结符里， 则X属于first(X)
            ...
            # 对当前X每个推导都处理完后，把每个推导的first集放到first（x）中
            for derivation in self.grammar[X]: #遍历每一个推导
        ...
        for key in self.grammar.keys():
            ...
            for i in range(alphaLen - 1):
                for j in range(i + 1, alphaLen):
                    # 如果当前元素的first集合和他后面的first集合相等,就不是LL1文法
    ...
    def genAnalysisTable(self):
        ...
        # print('预测分析表如下:')
        for nT in self.NonTerminater: #竖列
